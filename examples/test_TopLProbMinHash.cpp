/**
 * test_TopLProbMinHash – benchmark harness for ProbMinHash4 with Top-L
 * truncation (Route C).
 *
 * Identical to test_ProbMinHash except for the extra max_L parameter.
 *
 * Usage:
 *   exe_test_TopLProbMinHash <file_list> <dist_threshold> <threads>
 *                            [max_L=4] [max_bucket=500]
 *
 * max_L = 0 → original ProbMinHash4 (no truncation)
 * max_L = 1 → only first register update per element (fastest)
 * max_L = 2,4,8,… → trade-off between speed and accuracy
 */
...
int main(int argc, char* argv[])
{
    if (argc < 4) {
        cerr << "usage: " << argv[0]
             << " <file_list> <dist_threshold> <threads>"
                " [max_L=4] [max_bucket=500]"
             << endl;
        return 1;
    }
    ...
    const uint32_t MAX_L    = (argc >= 5) ? (uint32_t)stoi(argv[4]) : 4;
    const int MAX_BUCKET    = (argc >= 6) ? stoi(argv[5]) : 500;
    ...
    cerr << "===== total files: " << n
         << "  (ProbMinHash4 Top-L, L=" << (MAX_L == 0 ? "unlimited" : to_string(MAX_L))
         << ")" << endl;
    ...
    for (int t = 0; t < n; t++) {
        ...
        Sketch::ProbMinHash4 sk(M, KSIZE, SEED, MAX_L);
    ...
    string outname = string("res_dir/res.dist.TopL") +
                     (MAX_L == 0 ? "inf" : to_string(MAX_L)) + ".ProbMinHash";
    FILE* fp_out = fopen(outname.c_str(), "w");
    for (int t = 0; t < numThreads; t++)
