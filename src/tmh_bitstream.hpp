...
#ifndef _RABBITSKETCH_TMH_BITSTREAM_HPP_
#define _RABBITSKETCH_TMH_BITSTREAM_HPP_

#include "wyhash_v4_tmh.h"
...
static_assert(std::numeric_limits<double>::is_iec559,
              "Require std::numeric_limits<double>::is_iec559 to be true!");
...
template <typename T>
bool getBernoulli(double successProbability, T &bitstream) {
  while (true) {
...
bool getBernoulli(V successProbabilityNumerator,
                  const V successProbabilityDenominator, T &bitstream) {
  assert(successProbabilityDenominator > 0);
  assert(successProbabilityNumerator <= successProbabilityDenominator);
  while (true) {
    if (successProbabilityNumerator == 0)
      return false;
    if (successProbabilityNumerator == successProbabilityDenominator)
      return true;
    ...
    if (b)
      successProbabilityNumerator -= successProbabilityDenominator;
  }
}
...
static uint8_t getLog2Base(uint32_t v) {
  assert(v > 0);
  return static_cast<uint8_t>(std::numeric_limits<unsigned long>::digits -
                              __builtin_clzl(v));
}
#else
...
// Lumbroso, Jeremie. "Optimal discrete uniform generation from coin flips, and
// applications." arXiv preprint arXiv:1304.1916 (2013). This method is very
// efficient in terms of consumed random bits.
...
// Transactions on Modeling and Computer Simulation (TOMACS) 29.1 (2019): 3.
...
public:
  ...
  WyrandBitStream(uint64_t value, uint64_t seed)
      : state(wyhash64(value, seed)), hashBits(0), availableBits(0) {}

  WyrandBitStream(uint64_t value1, uint64_t value2, uint64_t seed)
      : hashBits(0), availableBits(0) {
    uint64_t data[2];
    data[0] = value1;
    data[1] = value2;
...
public:
  TruncatedExponentialDistribution(double rate)
      : rate(rate), c1((rate != 0) ? expm1(rate) / rate : 1),
  ...
  template <typename T> double operator()(T &bitstream) const {
...
public:
  PermutationStream(uint32_t _size)
      : size(_size), idx(0), versionCounter(0),
        permutationAndVersion(new std::pair<uint32_t, uint32_t>[_size]) {}

  bool hasNext() const { return idx < size; }

  template <typename H> uint32_t next(H &hashBitStream) {
    ...
    const uint32_t result = (permutationAndVersionK.second != versionCounter)
                                ? k
                                : permutationAndVersionK.first;
    const uint32_t x = (permutationAndVersionIdx.second != versionCounter)
                           ? idx
                           : permutationAndVersionIdx.first;
    permutationAndVersionK = std::make_pair(x, versionCounter);
    idx += 1;
    return result;
  }

  // must be called first before iterating over a new permutation using
  // next-method
  void reset() {
    idx = 0;
    if (versionCounter == 0) {
...
#endif // _RABBITSKETCH_TMH_BITSTREAM_HPP_
